...
ALL_TEST_PODS = TEST_PLACEMENT_PODS + ("helm-guard-cube-node-runtime-test",)
...
def assert_no_test_pods():
    for name in ALL_TEST_PODS:
        if pod_doc(name, required=False) is not None:
            raise SystemExit(f"{name} must be omitted when helmTest.enabled=false")
...
if mode == "default.yaml":
    ...
    for name in (
        ...
        check_test_placement(name)
    assert_absent("helm-guard-cube-node-runtime-test")
    ...
    check_test_placement("helm-guard-cube-node-image-test")
    check_compute_placement("helm-guard-cube-node-runtime-test")
    ...
    check_compute_placement(
        "helm-guard-cube-node-runtime-test", require_control_taint=True
    )
